...
cityInput.addEventListener("keyup", function (event) {
  if (event.key === "Enter") {
    ...
    var apiKey = "79a39cbed8daf68f394e601b05e7d9d2"; 
    var unit = "metric";
    ...
    if (cityInputValue != "") {
      async function getWeather() {
        ...
        if (data.message != "city not found" && data.cod != "404") {
          ...
          fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${cityInputValue}&appid=${apiKey}&units=${unit}`)            .then(response => response.json())
            .then(data => {
